...
extern void set_song_params(int clock_rate, int frame_count, double frame_rate);
...
void fxm_get_songinfo(char* songName, char *songAuthor, int maxlen) {
	songName[0]= songAuthor[0]= 0;	// reset
	if (_song_name) {
		strncpy(songName, _song_name, maxlen );
	}
	if (_song_author) {
		strncpy(songAuthor, _song_author, maxlen );
	}
}
...
void fxm_init(const uint8_t *fxm, uint32_t len)
{
  for (int i = 0 ; i < 3 ; ++i)
  {
